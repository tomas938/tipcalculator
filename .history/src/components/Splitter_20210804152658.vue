		...
		<div class="calculator">
			<div class="up">
				...
				<div class="input-wrapper">
					<input type="text" v-model="bill" placeholder="0.00" />
				...
				<div class="calculator__grid">
					<div class="calculator__grid__item item-1" @click="calcTip(0.05)">
						<p>5%</p>
					</div>
					<div class="calculator__grid__item item-2" @click="calcTip(0.1)">
						<p>10%</p>
					</div>
					<div class="calculator__grid__item item-3" @click="calcTip(0.15)">
						<p>15%</p>
					</div>
					<div class="calculator__grid__item item-4" @click="calcTip(0.25)">
						<p>25%</p>
					</div>
					<div class="calculator__grid__item item-5" @click="calcTip(0.5)">
						<p>50%</p>
					</div>
					<div class="calculator__grid__item item-6">
						<input
							type="text"
							placeholder="Custom"
							v-model="customTip"
							@keyup="calcCustomTip(customTip)"
						/>
					</div>
				</div>
				<div class="error-wrapper">
					<h3>Number of People</h3>
					<h3 class="error" v-show="+numberOfPeople === 0">Can't be zero</h3>
				</div>
				<div class="input-wrapper">
					<input
						:class="{ red: +numberOfPeople === 0 }"
						type="text"
	...
	methods: {
		calcTip(amount) {
			if (this.numberOfPeople > 0) {
				this.tip = ((this.bill * amount) / this.numberOfPeople).toFixed(2);
			} else {
				this.showError = true;
			}
		},
		calcCustomTip(customtip) {
			if (this.numberOfPeople > 0) {
				this.tip = (
					(this.bill * (customtip * 0.01)) /
					this.numberOfPeople
				).toFixed(2);
			} else {
				console.log("Enter at least one person");
			}
		},
		reset() {
			this.tip = null;
...
.red {
	border: 2px solid rgb(235, 63, 63);
	&:focus,
	&:active {
		border: 2px solid rgb(235, 63, 63);
	}
